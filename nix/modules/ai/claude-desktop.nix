# Configure Claude Desktop with shared MCP servers; only valid on macOS.
{
  ...
  # Wrap Claude Desktop to add shared tools to PATH
  wrappedClaudeDesktop = mcp.wrapWithTools {
    package = inputs.nix-ai-tools.packages.${system}.claude-desktop;
    binName = "claude-desktop";
  };
in {
  ...
  config = lib.mkIf ((hostConfig.os or null) == "linux") {
    ...
    home.packages = [wrappedClaudeDesktop];
