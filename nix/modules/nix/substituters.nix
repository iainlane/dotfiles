...
{lib, ...}: let
  ...
  # Allow admin users on both Linux and Darwin to drive nix-daemon operations
  # that require trusted privileges (e.g. deploy-rs store path uploads).
  trusted-users = [
    "root"
    "@sudo"
    "@admin"
...
in {
  config = {
    ...
    _module.args.substitutersCustomConf = ''
      ...
      trusted-users = ${lib.concatStringsSep " " trusted-users}
