...
# Linux system configuration via system-manager.
#
# system-manager is a tool that applies NixOS-style configuration to non-NixOS
# Linux systems. It can manage systemd services, /etc files, and other system
# state, but has some limitations compared to full NixOS:
#
# - No security.wrappers (we use a systemd service to set capabilities instead)
# - No direct kernel/boot configuration
# - Nix daemon is managed separately (we use Determinate Nix)
#
# This file works around these constraints while providing a consistent
# configuration experience across all Linux hosts.
{
  imports = [
