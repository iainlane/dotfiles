{
  flake.profiles.containers.os.linux.homeManagerModule = {inputs, ...}: {
    imports = [./podman-install-quadlet.nix];

    disabledModules = [
      "${inputs.home-manager}/modules/services/podman/linux/install-quadlet.nix"
    ];
